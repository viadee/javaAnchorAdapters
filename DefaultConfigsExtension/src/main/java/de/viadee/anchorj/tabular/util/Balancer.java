...
public final class Balancer {
    ...
    public static TabularInstance[] balance(TabularInstance[] instances, Random rnd) {
        ...
        if (Stream.of(instances).map(TabularInstance::getOriginalLabel).anyMatch(Objects::isNull)) {
        ...
        final Map<Integer, Integer> countPerLabel = Stream.of(instances)
                .collect(Collectors.groupingBy(TabularInstance::getOriginalLabel,
        ...
        for (TabularInstance instance : list) {
            final Integer currentCount = addedCount.getOrDefault(instance.getOriginalLabel(), 0);
            if (currentCount < minCount) {
                result.add(instance);
                addedCount.put(instance.getOriginalLabel(), currentCount + 1);
            }
        }
